    ...
    try {
      ...
      if (alreadyLiked) {
        ...
        const event = await Event.findByIdAndUpdate(
          ...
          { $push: { atendants: req.user._id } },

          { new: true }
        );
        await User.findByIdAndUpdate(
          req.user._id,
          { $push: { registered: req.params._id } },
          { new: true }
        );
      ...
      res
        .status(500)
        .send({ message: 'There was a problem with your registration' });
    }
  },
  ...
  async unregister(req, res) {
    try {
      ...
      if (alreadyLiked === false) {
        return res
          .status(400)
          .send({ message: 'You have not registered for this event' });
      }

      const event = await Event.findByIdAndUpdate(
        findEvent._id,
        { $pull: { atendants: req.user._id } },
        { new: true }
      );

      await User.findByIdAndUpdate(
        req.user._id,
        { $pull: { registered: req.params._id } },
        { new: true }
      );
      ...
      res
        .status(500)
        .send({ message: 'There was a problem removing your registration' });
    }
  },
